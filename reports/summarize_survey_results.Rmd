...
```{r}
df <-
  read_csv(here("data", "ChildYouthMentalHeal_DATA_2022-10-06_1209.csv"), na = c("", "NA", "N/A", "none", "?")) %>%
...
exclude_age <-
  ...
  mutate(exclude_age = case_when(!is.na(survey_04_exclude_young) & !is.na(survey_04_exclude_old) ~ glue("{survey_04_exclude_young} to {survey_04_exclude_old}"),
...
q6_results <-
  ...
  mutate(survey_06 = recode(survey_06,
                            ...
                            "6" = "12+ months") %>%
           as_factor() %>%
           fct_relevel(c("< 1 month", "1-2 months", "3-6 months", "7-9 months", "10-12 months", "12+ months")))
...
q15 <-
  ...
  mutate(survey_15_recode = recode(survey_15,
                                   ...
                                   "4" = ">20 min") %>%
           as_factor() %>%
           fct_relevel(c("5-10 min", "10-15 min", "15-20 min", ">20 min")))
